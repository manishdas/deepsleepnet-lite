...
DeepSleepNet~\cite{Supratak2017} pioneered a dual-path CNN with small filters ($\sim$0.5\,s) for high-frequency features and large filters ($\sim$4\,s) for slow waves, followed by a BiLSTM. DeepSleepNet-Lite~\cite{Fiorillo2019} simplified this for lower cost while preserving the dual-path concept. Our baseline adopts this philosophy.

\paragraph{Temporal and attention-based methods.}
LSTMs~\cite{Hochreiter1997} and bidirectional variants have been widely used for sleep stage transitions. More recently, AttnSleep~\cite{Eldele2021} applied multi-head attention over CNN features, and SleepTransformer~\cite{Phan2022} used epoch- and sequence-level transformers. The Conformer~\cite{Gulati2020}, originally for speech recognition, combines self-attention with depthwise convolution. We adapt it for EEG epoch sequences.

\paragraph{Training techniques for imbalanced data.}
Focal loss~\cite{Lin2017} down-weights easy examples to focus on hard cases. Mixup interpolates training pairs for smoother decision boundaries. We combine these with a three-stage training pipeline and cosine annealing with warmup.
...
\begin{figure}[t]
  ...
  \caption{Per-class F1 comparison between CNN+BiLSTM and CNN+Conformer across 20-fold LOSO-CV. The BiLSTM leads on all stages except N2 (tied).}
